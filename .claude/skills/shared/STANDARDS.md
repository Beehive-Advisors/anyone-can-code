...
**Preferable, not strictly required.** Student work should happen inside the lab's own folder (e.g., `1.5/`) rather than in `/tmp` or the user's home directory. Any files the student creates (scratch text files), any files they inspect (a sample PNG, a sample binary), and any artifacts they'll throw away when the lab ends should live within that folder.
...
- Cleanup at the end of the lab stays simple. Remove the files the student created; leave the shipped reference files (like `sample.png` in Lab 1.5) in place.
- Output blocks in the lab become deterministic. Every student running `xxd -l 16 sample.png` sees identical bytes because the file is bit-for-bit the same everywhere.

When leaving the folder is fine:

- Reading system files by absolute path (e.g., `xxd -l 4 /bin/ls`, `cat /proc/cpuinfo`, `sysctl -n hw.memsize`). Those aren't "student artifacts" — they're points of interest elsewhere in the OS, and the commands work from any working directory.
- Labs whose topic *is* the filesystem convention being taught. If a future lab teaches `/tmp` or `/var/log`, working there is correct.
- Demos that genuinely require a specific system location (installing a service, editing a system config).
...
- Keep a short `cd`-teaching-moment the first time the student's working directory matters (see Lab 1.5 Part 4 for an example): explain that `cd` is sticky, that `pwd` prints the current directory, and that `cd` with no argument returns home.
- Cleanup command at the end of the lab removes student-created files only — the shipped references stay.
